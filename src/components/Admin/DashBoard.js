...
import { Row, Col, Icon, Card, Modal, Divider, Table, Tag, Button, Input } from "antd";
...
export default function DashBoard() {
    ...
    const confirmAuthUser = (userId) => {
        axios.get('http://13.124.113.72:8080/authImage/confirm?userId=' + userId)
            .then((response) => {
            ...
            >
                <div>
                    ...
                    <Row style={{marginTop : "40px"}}>
                        <Col span={8} offset={8}>
                            <Button onClick={() => { confirmAuthUser(userId) }}>학생증 인증 확인</Button>
                        </Col>
                    </Row>
                </div>
            </Modal>
        ...
        },
        {
            ...
            render: (flag, record) => flag ? <a>인증 완료</a> : <div><Button
                ...
                {userCampusAuthImage && userCampusAuthImage.authImageUrl ?
                    authImageConfirmModal(userCampusAuthImage.authImageUrl, userCampusAuthImage.userId) : null}
                ...
                !isAuth && !sessionStorage.getItem('isAdmin')  ?
                    <Row style={{margin : "20% auto 20% auto"}}>
                        <Col span={6} offset={8}>
                            ...
                            onPressEnter={(e)=>{if(e.target.value === "3428") {
                                setIsAuth(true);
                                sessionStorage.setItem('isAdmin', true);
                                }}} />
                        </Col>
                        <Col span={2} offset={0}>
                            <Button onClick={(e)=>{if(e.target.value === "3428") {
                                setIsAuth(true);
                                sessionStorage.setItem('isAdmin', true);
                                }}}>인증</Button>
                        </Col>
