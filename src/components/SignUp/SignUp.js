...
class SignUp extends Component{
  ...
  constructor(props) {
    ...
    validateEmail = async (email) => {
      ...
      axios.get('http://13.125.191.60:8080/users/signup/validateEmail', {
        params: {
          email : email
        }
      })
        .then((response) => {
          if(response.data.message == "이미 존재하는 Email입니다."){
            this.setState({
              authEmail : false             
            });
          }
    ...
    fetchUserInfo = async (res) => {
      ...
      axios.post('http://13.125.191.60:8080/users/signup', {
        name : res.name,
        email : res.email,
    ...
    componentDidMount() {
      ...
      render() {
        ...
        if (this.state.completeToggle) {
          return <Redirect to="/" />;
        }

        return (
          <Container class="signup" fluid>
            <Navbar></Navbar>
            <Row>
            <Col xs sm md lg ={6} className="right-col">
                  <h2 class="signuptext left">이용 약관</h2>
                  <TextArea rows={10} 
                  value = {agreeText}/>
              ...
              <Col xs sm md lg ={6} className="left-col">
              <h2 class="signuptext">회원 가입</h2>
              <Form className="signUpForm"
                {...formItemLayout}>
                    <Form.Item label="이름">
                      ...
                      <Form.Item label="전공">
                        {getFieldDecorator('major', {
                          ...
                          rules: [
                            { type: 'array', required: true, message: '해당되는 전공을 선택해주세요!' },
                          ],
                    ...
                    <Button className = "submit-button" type="primary" htmlType="submit" onClick={(e) => {
